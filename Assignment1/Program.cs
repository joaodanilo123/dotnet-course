...
app.Run(async (HttpContext context) =>
{
    ...
    else if (context.Request.Path.StartsWithSegments("/employees"))
    {
        ...
        if (context.Request.Method == "GET")
        {
            ...
            {
                context.Response.Headers["Content-Type"] = "text/html";
                var parsed = int.TryParse(context.Request.Query["id"], out var id);
                if(parsed)
                {
                    var employee = EmployeesRepository.GetEmployee(id);
                    if(employee is not null)
                    {
                        context.Response.StatusCode = 200;
                        await context.Response.WriteAsync($"<li><b>{employee.Name}</b>: {employee.Position} ({employee.Salary})</li>");
                    } else
                    {
                        context.Response.StatusCode = 404;
                        await context.Response.WriteAsync($"<li>Employee not found.</li>");
                    }
                ...
                else
                {
                    ...
                    await context.Response.WriteAsync("<p>Invalid Parameter</p>");
                }
